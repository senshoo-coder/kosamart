...
export default function AdminDashboard() {
  ...
  return (
    <div className="p-5 space-y-5">
      <div>
        <h1 className="text-[20px] font-bold text-[#1a1c1c]">관리자 대시보드</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { href: '/admin/users?status=pending', icon: '👥', label: '유저 승인', color: '#ede9fe', accent: '#6d28d9' },
          { href: '/admin/orders?status=pending', icon: '⚡', label: '빠른 승인', color: '#fef3c7', accent: '#b45309' },
          { href: '/admin/delivery', icon: '🗺️', label: '배달 관제', color: '#dbeafe', accent: '#1d4ed8' },
          { href: '/admin/products', icon: '🏷️', label: '공구 관리', color: '#d1fae5', accent: '#10b981' },
        ].map(item => (
          <Link key={item.href} href={item.href}>
            <div className="bg-white rounded-[8px] p-4 flex items-center gap-3 hover:shadow-md transition-shadow cursor-pointer" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              <div className="w-10 h-10 rounded-[10px] flex items-center justify-center text-xl" style={{ background: item.color }}>
                {item.icon}
              </div>
              <p className="text-[13px] font-semibold text-[#1a1c1c]">{item.label}</p>
            </div>
          </Link>
        ))}
      </div>
      ...
      <div className="bg-white rounded-[8px] overflow-hidden" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <div className="px-5 py-4 flex items-center justify-between border-b border-[#f5f5f5]">
          <h2 className="text-[14px] font-bold text-[#1a1c1c]">최근 주문 (전체)</h2>
          <Link href="/admin/orders"><span className="text-[12px] text-[#6d28d9]">전체 보기 →</span></Link>
        </div>
        {loading ? (
                ...
                <div className="px-5 py-3.5 flex items-center gap-4 hover:bg-[#f9f9f9] transition-colors cursor-pointer">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-[#a3a3a3] mt-0.5 truncate">
                      {order.store_name ? `🏪 ${order.store_name}` : order.order_number}
                    </p>
                  </div>
