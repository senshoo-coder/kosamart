  ...
  return (
    <div className="p-5 max-w-3xl mx-auto space-y-4">
      ...
      <div className="bg-white rounded-[8px] p-5" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <h2 className="text-[13px] font-semibold text-[#1a1c1c] mb-3">고객 정보</h2>
      ...
      {(order.status === 'pending' || order.status === 'paid') && (
        <div className="bg-white rounded-[8px] p-5" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
          <label className="text-[13px] font-semibold text-[#1a1c1c] mb-2 block">관리자 메모 (선택)</label>
          <textarea
            value={ownerMemo}
      ...
      {order.owner_memo && order.status !== 'pending' && order.status !== 'paid' && (
        <div className="bg-[#fef3c7] rounded-[8px] p-4 border border-[#fde68a]">
          <p className="text-[11px] text-[#b45309] font-semibold mb-1">관리자 메모</p>
      ...
      {rejectModal && (
        ...
        >
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" />
          <div className="relative w-full max-w-sm bg-white rounded-[16px] p-6">
            <h3 className="text-[16px] font-bold text-[#1a1c1c] mb-1">주문 취소/거절</h3>
            <p className="text-[11px] text-[#a3a3a3] mb-4 font-mono">{order.order_number}</p>
            <textarea
              value={rejectReason}
              onChange={e => setRejectReason(e.target.value)}
              placeholder="취소/거절 사유 입력 (필수)"
            ...
            <div className="flex gap-3 mt-4">
              ...
              <Button variant="danger" className="flex-1" onClick={handleReject} loading={actionLoading}>확정</Button>
            </div>
          </div>
        </div>
      )}
