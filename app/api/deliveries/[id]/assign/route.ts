...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!driver) return NextResponse.json({ data: null, error: '배달맨 인증 실패' }, { status: 401 })

  const { data, error } = await supabase
    .from('deliveries')
    .update({ driver_id: driver.id, status: 'assigned', assigned_at: new Date().toISOString() })
    .eq('id', id)
    .eq('status', 'pending')   // 이미 다른 배달맨이 수락한 경우 업데이트 안 됨
    .select(`*, order:orders(store_id, order_number, kakao_nickname, delivery_address)`)
    .single()

  if (error || !data) {
    return NextResponse.json({ data: null, error: '이미 다른 배달맨이 수락한 배달입니다' }, { status: 409 })
  }
